...
DP_container_2D = jd.DeformableParticleContainer.create(
    vertices=state2D.pos,
    elements=elements,
...
state2D, system2D, (state_traj, system_traj) = system2D.trajectory_rollout(
    state2D, system2D, n=n_snapshots, stride=save_stride
)

print(state2D.force)

animate(state_traj, system_traj, 'test.gif')
