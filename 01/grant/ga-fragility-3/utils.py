...
def step(cfg, input_path, state = None, system = None, use_dynamic_rollout = False):
    ...
    if use_dynamic_rollout:
        ...
        msd_res = evaluate_binned(msd_kernel, {"pos": pos_u}, bins)
        msd = np.asarray(msd_res.mean)

        k = 2.0 * jnp.pi / 1.0
        isf_res = evaluate_binned(isf_self_isotropic_kernel, {"pos": pos_u}, bins, kernel_kwargs={"k": k})
        isf = np.asarray(isf_res.mean)

        tau_steps = bins.values()
        t = tau_steps * float(system.dt)

        # save processed traj + energies (move to host first if your saver expects numpy)
        pe, ke, ke_r, vel_u, angVel_u, q_w_u, q_xyz_u, pos_u = jax.device_get(
            (pe, ke, ke_r, vel_u, angVel_u, q_w_u, q_xyz_u, pos_u)
        )

        save_arrs(
            [pe, ke, ke_r, pos_u, q_w_u, q_xyz_u, vel_u, angVel_u, msd, isf, t],
        ...
        t = bins.values() * cfg.min_save_decade * float(system.dt)
