...
def create_ga_clumps_2d(N, mu, nv, aspect_ratio, mass, e_int, phi, dt):
    dim = 2
    particle_radii = jd.utils.dispersity.get_polydisperse_radii(N)
    asperity_radius = get_closest_vertex_radius_for_mu_eff(mu, min(particle_radii), nv)
    max_nv, max_mu_eff, err = find_num_vertices_for_target_mu_eff(mu, asperity_radius, max(particle_radii))
    vertex_counts = np.ones_like(particle_radii).astype(int) * nv
