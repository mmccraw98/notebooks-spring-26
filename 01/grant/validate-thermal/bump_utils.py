...
def create_clumps_2d(phi, N, mu_eff, aspect_ratio, min_nv, mass):
    dim = 2
    ...
    sphere_phi = jnp.sum(jnp.pi * state.rad ** 2) / jnp.prod(box_size)
    skin = (2e-2 / sphere_phi) ** (1 / state.dim)  # empirical
...
def create_clumps_3d(phi, N, asperity_radius, aspect_ratio, min_nv, mass):
    dim = 3
    ...
    sphere_phi = jnp.sum(4 / 3 * jnp.pi * state.rad ** 3) / jnp.prod(box_size)
    skin = (2e-2 / sphere_phi) ** (1 / state.dim)  # empirical
    cutoff = 2.0 * jnp.max(state.rad)
...
def create_spheres(phi, N, dim, mass, seed=None):
    e_int = 1.0
    dt = 1e-2
    if dim == 2:
        cr = [0.5, 0.5]
