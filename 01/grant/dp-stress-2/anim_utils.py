...
def animate(traj_state, traj_system, path, frames=100, fps=15):
    animate_from_data(np.asarray(traj_state.pos), np.asarray(traj_state.rad), np.asarray(traj_state.ID), np.asarray(traj_system.domain.box_size), path, frames, fps)

def animate_from_data(pos, rad, pids, box_size, path, frames, fps):
    import subprocess
    import h5py
    with h5py.File("traj.h5", "w") as f:
        f.create_dataset("pos", data=pos)
        f.create_dataset("rad", data=rad)
        f.create_dataset("ID", data=pids)
