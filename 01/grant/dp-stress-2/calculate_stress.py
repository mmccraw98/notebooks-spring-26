...
root = f'/home/mmccraw/dev/data/26-01-01/grant/dp-fragilitiy/version-5/compression-4/{name}'
...
for phi_dir in tqdm(os.listdir(root)):
    ...
    def save_fn(st, sy):
        bonded_force, _ = system.bonded_force_model.compute_forces(st.pos, st, sy)
        bonded_virial = jnp.sum(
            st.pos[:, :, None] * bonded_force[:, None, :], axis=0
        )
    ...
    step_ids, bonded_virial, contact_virial, kinetic_virial = logged

    os.makedirs(f'virials-4/{name}', exist_ok=True)
    np.savez(
        f'virials-4/{name}/{phi_dir}.npz',
        step_ids=step_ids,
