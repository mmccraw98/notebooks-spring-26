...
def step(cfg, input_path, state = None, system = None, dp = None, use_dynamic_rollout = False):
    ...
    if use_dynamic_rollout:
        ...
        def save_fn(st, sy):
            return (
                sy.step_count,
                get_com_pos(st, N_dps=N_dps),
                get_com_vel(st, N_dps=N_dps),
        ...
        (step_ids, pos_dp, vel_dp, pe, ke) = logged
        ...
        pe, ke, pos_dp, vel_dp = jax.device_get((pe, ke, pos_dp, vel_dp))
        ...
        for i in range(n_chunks):
            ...
            pe_chunks.append(jax.vmap(jd.utils.thermal.compute_potential_energy)(state_traj, system_traj))
            ke_chunks.append(jax.vmap(jd.utils.compute_translational_kinetic_energy)(state_traj))
            pos_dp_chunks.append(jax.vmap(lambda st: get_com_pos(st, N_dps=N_dps))(state_traj))
            vel_dp_chunks.append(jax.vmap(lambda st: get_com_vel(st, N_dps=N_dps))(state_traj))

        pos_dp = jnp.concatenate(pos_dp_chunks)
        vel_dp = jnp.concatenate(vel_dp_chunks)
        pe = jnp.concatenate(pe_chunks)
        ke = jnp.concatenate(ke_chunks)
        print('Done')

        T = pos_dp.shape[0]
        bins = LagBinsPseudoLog(T, dt_min=1, dt_max=T - 1)
        msd_res = evaluate_binned(msd_kernel, {"pos": pos_dp}, bins)
        msd = np.array(msd_res.mean)
        k = 2.0 * jnp.pi / 1.0
        isf_res = evaluate_binned(isf_self_isotropic_kernel, {"pos": pos_dp}, bins, kernel_kwargs={"k": k})
        isf = np.array(isf_res.mean)
        tau_steps = bins.values()
        t = tau_steps * cfg.min_save_decade * float(system.dt)
    ...
    return state, system, dp, state_traj, system_traj, pe, run_root
