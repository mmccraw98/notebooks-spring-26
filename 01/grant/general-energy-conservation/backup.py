...
def measure_energy_conservation_spheres(phi, N, mass, target_temperature, dim, e_int, collider_type, domain_type):
    ...
    for dt, n_step, save_stride in zip(dts, n_steps, save_strides):
        ...
        if collider_type == "naive":
            ...
            raise ValueError(f'Unkown collider_type: {collider_type}')

        if domain_type == 'free':
            domain_kw = dict()
        elif domain_type == 'periodic':
            domain_kw = dict(box_size=box_size)
        elif domain_type == 'reflect':
            domain_kw = dict(box_size=box_size)
        elif domain_type == 'reflectsphere':
            domain_kw = dict(box_size=box_size)
        else:
            raise ValueError(f'Unkown domain_type: {domain_type}')
...
def measure_energy_conservation_clumps(phi, N, mass, target_temperature, dim, e_int, Nv, asperity_radius, collider_type, domain_type):
    save_stride_max = 1_000
    n_steps_max = 100_000
    dts = jnp.array([1e-3, 3e-3, 1e-2])
    n_steps = (n_steps_max * dts.min() / dts).astype(dtype=type(n_steps_max))
    ...
    for dt, n_step, save_stride in zip(dts, n_steps, save_strides):
        ...
        ke_r = jax.vmap(lambda st: calc_rot_ke(st, n_clumps=particle_radii.size))(state_traj)
        pe = jnp.sum(
            jax.vmap(
                lambda st, sys:
                sys.collider.compute_potential_energy(st, sys))(state_traj, system_traj),
        ...
        fluctuation.append(jnp.std(ke_t + ke_r + pe) / jnp.mean(ke_t + ke_r + pe))

    fluctuation = jnp.array(fluctuation)
    exponent, _ = np.polyfit(np.log(dts), np.log(fluctuation), 1)
    print(f"Total Energy Fluctuation ~ dt^{exponent:.2f}.  Expected: dt^{EXPECTED_EXPONENT}")

def measure_energy_conservation_dps(phi, N, mass, target_temperature, dim, e_int, e_m, e_b, e_c, Nv, asperity_radius, collider_type, domain_type):
    save_stride_max = 1_000
    n_steps_max = 100_000
    dts = jnp.array([1e-3, 3e-3, 1e-2])
    n_steps = (n_steps_max * dts.min() / dts).astype(dtype=type(n_steps_max))
...
if __name__ == "__main__":
    ...
    def test_clumps():
        ...
        for collider_type in ["naive", "neighborlist"]:
            print(f'\nRunning {collider_type}')
            measure_energy_conservation_clumps(
                phi, N, mass, target_temperature, dim, e_int, Nv, asperity_radius,
                collider_type=collider_type,
                domain_type=domain_type,
            )
    test_clumps()
