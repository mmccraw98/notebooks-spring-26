...
if __name__ == "__main__":
    ...
    data_root = '/home/mmccraw/dev/data/26-01-01/grant/debugging-cell-list/3d'
    figures_root = '/home/mmccraw/dev/analysis/spring-26/01/grant/debugging-cell-list/figures/energies3d'
    ...
    for asperity_radius in [0.4]:
        for cutoff_multiple in [1.001, 1.01, 1.1, 1.5, 2.0]:
            name = f'rad-{asperity_radius}-cm-{cutoff_multiple}'
            ...
            state, system = create_clumps_3d(0.5, N, asperity_radius, 1.0, 20, 1.0, cutoff_multiple)
            state = jd.utils.thermal.set_temperature(state, 1e-3, is_rigid=True, subtract_drift=True, seed=1)
            print(jnp.floor(system.domain.box_size / system.collider.cell_size).astype(int))
