...
if __name__ == "__main__":
    ...
    def get_neighbors_for_particle(pid_i, pos_i, ID_i, neighbor_cell_hashes_i):
        def body_fun(stencil_id, carry):
            ...
            def body_fun(val):
                ...
                d_sq = jnp.sum(dr ** 2)
                valid_neighbor = (d_sq <= cutoff_sq) * (ID_i != sorted_clump_ids[k])
                neighbor_id = sorted_particle_ids[k] * valid_neighbor + (valid_neighbor - 1)
                neighbor_list = neighbor_list.at[neighbor_count].set(neighbor_id)  # use the unsorted ids for both
    ...
    neighbor_list = jax.vmap(get_neighbors_for_particle)(sorted_particle_ids, sorted_pos, sorted_clump_ids, neighbor_cell_hashes)
    
    # unsort the neighbor list
    inv_perm = jnp.empty_like(perm)
    inv_perm = inv_perm.at[perm].set(jnp.arange(perm.shape[0], dtype=perm.dtype))
    neighbor_list = neighbor_list[inv_perm]
    ...
    def per_particle_force(i, pos_p_i, neighbors):
        def neighbor_force(j):
            valid = j != -1
            safe_j = jnp.maximum(j, 0)
            f, t = system.force_model.force(i, safe_j, state, system)
            return f * valid, t * valid
        forces, torques = jax.vmap(neighbor_force)(neighbors)
        f_sum = jnp.sum(forces, axis=0)
        t_sum = jnp.sum(torques, axis=0) + jnp.cross(pos_p_i, f_sum)
        return f_sum, t_sum
    
    force, torque = jax.vmap(per_particle_force)(iota, pos_p_global, neighbor_list)
    state.force += force
    state.torque += torque

    print(jnp.sum(force, axis=0))
    print(jnp.sum(torque, axis=0))

    # DPS HAVE SELF INTERACTION
    # DPS WONT BATCH
